...
def main(server='na'):
  ...
  {{% block body %}}
      <section>
        ...
        Sort by: <a href="./?server={server}">Alphabetically</a> |
        <a href="?sort_by=tier&server={server}">Tier</a> |
        <a href="?sort_by=nation&server={server}">Nation</a> |
        <a href="?sort_by=type&server={server}">Class</a>
        <br />
        {tanks}
      </section>
  {{% endblock %}}
  ...
  nations = {nation[0].lower() for nation in c.fetchall()}
  ...
  for tank_id, tank_name, *_ in valid_tanks:
    tank_html.append('''<li><a href='{}/?server={}'>{}</a></li>'''.format(tank_id, server, tank_name))
  tank_html.append('</ul>')
  ...
  for tank_id, tank_name, foo, bar, tier in valid_tanks_tier:
    ...
    tank_html_tier.append('''<li><a href='{}/?server={}'>{}</a></li>'''.format(tank_id, server, tank_name))
  tank_html_tier.append('</ul>')

  for nation in nations:
    nation_entries = [(entry[0], entry[1]) for entry in valid_tanks_tier if entry[2].lower() == nation]
    ...
    for tank_id, tank_name in nation_entries:
      tank_html_nation.append('''<li><a href='{}/?server={}'>{}</a></li>'''.format(tank_id, server, tank_name))
  ...
  for ttype in tank_types:
    ...
    for tank_id, tank_name in type_entries:
      tank_html_class.append('''<li><a href='{}/?server={}'>{}</a></li>'''.format(tank_id, server, tank_name))
  ...
  for page, content in pages.items():
    with open('templates/' + server + '/' + page, 'w') as f:
