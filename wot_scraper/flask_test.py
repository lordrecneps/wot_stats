...
def dw_tank_stats(tank_id):
	if request.method == 'GET':
		...
		return flask.render_template('tanks/' + tank_id + '_' + page.lower() + '.html')
	else:
		return flask.render_template('tanks/' + tank_id + '_150.html')
	...
	if request.method == 'GET':
		...
		if r:
			...
			for row in tank_stats:
				...
				stats_html.append('<td><a href="../tanks/{}/?v={}">{}</a></td>'.format(row[7], top50_version, row[1]))
				for i in range(2, 4):
			...
			for row in tank_stats:
				...
				ps_html.append('<td><a href="../tanks/{}/?v={}">{}</a></td>'.format(row[8], top50_version, row[0]))
				for i in range(1, 3):
			...
			return flask.render_template(
				...
				player_id=r[0], player_stats=''.join(ps_html), stat_version=query[1]
			)
		else:
			con_pg.close()
